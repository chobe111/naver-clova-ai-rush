...
from model import EnsembleModelB
...
def bind_ensemble_nsml(model, optimizer, scheduler):
    def save(dir_name, *args, **kwargs):
        ...
        state = {
            ...
            'modelD': model.modelD.state_dict(),
            'modelE': model.modelE.state_dict()
    ...
    def load(dir_name, *args, **kwargs):
        ...
        model.modelD.load_state_dict(state['modelD'])
        model.modelE.load_state_dict(state['modelE'])
        # model.load_state_dict(state['model'])
        # model.modelA = state['model']
...
def get_model_A():
    ...
    model.classifier = nn.Sequential(
        nn.Linear(in_features=in_features_num,
                  out_features=in_features_num // 2),
        nn.ReLU(),
        nn.Dropout(p=0.4),
        nn.Linear(in_features=in_features_num // 2, out_features=num_classes)
    )
    ...
    bind_nsml(model, scheduler=scheduler, optimizer=optimizer)
    nsml.load(checkpoint='15', session='KR95344/airush2021-1-3/233')
    return model.cuda()
...
def get_model_B():
    ...
    bind_nsml(model, scheduler=scheduler, optimizer=optimizer)
    nsml.load(checkpoint='15', session='KR95344/airush2021-1-3/234')
...
def get_model_C():
    ...
    model.classifier = nn.Sequential(
        ...
        nn.ReLU(),
        nn.Dropout(p=0.4),
        nn.Linear(in_features=in_features_num // 2,
                  out_features=in_features_num // 4),
        nn.ReLU(),
        nn.Dropout(p=0.4),
        nn.Linear(in_features=in_features_num // 4,
                  out_features=in_features_num // 8),

        nn.ReLU(),
        nn.Linear(in_features=in_features_num // 8, out_features=num_classes)
    )
    ...
    bind_nsml(model, scheduler=scheduler, optimizer=optimizer)
    nsml.load(checkpoint='15', session='KR95344/airush2021-1-3/227')
...
def get_model_D():
    ...
    bind_nsml(model, scheduler=scheduler, optimizer=optimizer)
    nsml.load(checkpoint='15', session='KR95344/airush2021-1-3/226')
...
def get_model_E():
    ...
    bind_nsml(model, scheduler=scheduler, optimizer=optimizer)
    nsml.load(checkpoint='15', session='KR95344/airush2021-1-3/225')
...
if __name__ == '__main__':
    ...
    ensemble_model = EnsembleModelB(modelA, modelB, modelC, modelD, modelE)
    ...
    if IS_ON_NSML:
        bind_ensemble_nsml(
            ensemble_model, optimizer=optimizer, scheduler=scheduler)

    if config.pause:
        nsml.paused(scope=locals())
    # init_weight(model)
    if mode == 'train':
        nsml.save('ensemble_model')
    # get flops of model
