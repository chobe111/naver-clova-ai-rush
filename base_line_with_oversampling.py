...
from data_local_loader2 import OverSampleTrainDataset
...
if __name__ == '__main__':
    ...
    #                                                 steps_per_epoch=steps_per_epoch, epochs=num_epochs, anneal_strategy='cos')
    ...
    if mode == 'train':
        dataset = OverSampleTrainDataset(
            root=DATASET_PATH, batch_size=batch_size, is_train=True)

        tr_loader = dataset()
        total_data_len = len(dataset)
        steps_per_epoch = int(total_data_len / batch_size)
        best_acc = 0
