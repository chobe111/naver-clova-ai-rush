...
if __name__ == '__main__':
    ...
    if mode == 'train':
        ...
        vl_loader = data_loader(root=DATASET_PATH, phase='test',
                                split=train_split, batch_size=batch_size, submit=False)

        time_ = datetime.datetime.now()
        num_batches = len(tr_loader)

        validation_stat = AverageMeter()
        ...
        for epoch in range(num_epochs):
            ...
            model.train()
            for iter_, (data, val) in enumerate(zip(tr_loader, vl_loader)):
                global_iter += iter_
                _, x, label_0, label_1, label_2 = data
                _, val_x, v_label_0, v_label_1, v_label_2 = val
                ...
                if cuda:
                    ...
                    val_x = val_x.cuda()
                    v_label_0 = v_label_0.cuda()[:, 0]
                    v_label_1 = v_label_1.cuda()[:, 0]
                    v_label_2 = v_label_2.cuda()[:, 0]
                ...
                val_loss = loss_fn_0(val_pred, v_label_0)

                train_stat.update(loss.item(), x.size(0))
                validation_stat.update(val_loss.item(), val_x.size(0))
                ...
                if (iter_ + 1) % print_iter == 0:
                    ...
                    print('[{:.3f}/{:d}] val loss({}) '
                          'elapsed {} expected per epoch {}'.format(
                              _epoch, num_epochs, validation_stat.avg, elapsed, expected))
                    ...
                    if IS_ON_NSML:
                        ...
                        report_dict["validate_loss"] = float(
                            validation_stat.avg)
